...
export default async function WarrantPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="container form-container">
    ...
    </article><div className="hero-actions"><Link href={`/warrant/${FINAL_PROOF.lowWarrantId}`} className="button button--secondary">Final LOW proof</Link><Link href={`/warrant/${FINAL_PROOF.highWarrantId}`} className="button button--secondary">Final HIGH proof</Link><Link href="/compare" className="button button--secondary">Compare reliance →</Link></div>
  </div>;
}
