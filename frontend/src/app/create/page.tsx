  ...
  const renderLifecycleUI = () => {
    ...
    return <div className="protocol-panel mb-8">
      ...
      <div className="bg-bg-deep-graphite p-5 border border-rules text-xs font-mono text-text-secondary space-y-2">
        <div className="break-all"><span className="text-white">Warrant ID:</span> {generatedWarrantId}</div>
      ...
      {trackingError && <div className="error-notice">{trackingError}{!isTerminal && <button onClick={() => startTracking(tracked!)} className="block mt-2 underline">Retry Status Check</button>}</div>}
  ...
  return <div className="container form-container">
    ...
    {txHash ? renderLifecycleUI() : <form onSubmit={handleSubmit} className="glass instrument-form">
      ...
      <section className="form-stage"><header className="form-stage-header"><h2><span className="form-stage-number">03 /</span> How serious is the decision?</h2><span className="metadata">CONSEQUENCE SEVERITY</span></header>
        ...
        ].map(level => <button key={level.id} type="button" onClick={() => { setRisk(level.id); invalidate(); }} className="risk-option" aria-pressed={risk === level.id}><span>{level.id}</span><small>{level.desc}</small></button>)}</div>
      </section>
      ...
      <section className="form-stage"><header className="form-stage-header"><h2><span className="form-stage-number">04 /</span> Anything that must be proven?</h2><span className="metadata">OPTIONAL</span></header>
        <div className="field-group"><label htmlFor="requirements">Extra requirements, one per line</label><textarea id="requirements" value={reqs} onChange={event => { setReqs(event.target.value); invalidate(); }} rows={3} className="form-input" placeholder={"Example:\nMust establish ownership\nMust establish inspection status"}/></div>
        <p className="form-hint">Leave this blank if you do not have extra conditions.</p>
      </section>

      <section className="form-stage issue-stage"><div><header className="form-stage-header"><h2><span className="form-stage-number">05 /</span> Ask the validators</h2></header><p className="form-hint">Your wallet creates the warrant. The contract and validators decide the result.</p></div><button type="submit" disabled={isSubmitting || !url || !action || !prepared || !expectedHash} className="button button--primary">{isSubmitting ? "AWAITING WALLET / SUBMITTING..." : address ? "ISSUE RELIANCE WARRANT" : "CONNECT WALLET & ISSUE WARRANT"}<span aria-hidden="true">↗</span></button></section>
    </form>}
  </div>;
}
